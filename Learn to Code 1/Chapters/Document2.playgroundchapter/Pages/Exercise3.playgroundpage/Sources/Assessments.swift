...
public func assessmentPoint() -> AssessmentResults {
    ...
    var hints = [
        "First, [define](glossary://define) your `solveStair()` function to walk up a stairway, collect a gem, turn around, and move back to the center.",
    ...
    if checker.functionCallCount(forName: "solveStair") == 0 && checker.functionCallCount(forName: "turnAround") > 0 {
        ...
        hints[0] = "You called `turnAround()`, but you didn't [call](glossary://call) `solveStair()`. Use `turnAround()` to [define](glossary://define) `solveStair()`; then you can [call](glossary://call) `solveStair()` four times to collect the gem on each stairway."
        ...
        hints[0] = "You're on the right path! Figure out which commands to use to [define](glossary://define) `solveStair()`, then [call](glossary://call) that function multiple times for each stairway and collect all four gems."
    } else if checker.numberOfStatements > 16 && checker.functionCallCount(forName: "solveStair") < 1 {
        success = "You're getting there \nRemember, the reason to use functions is to break a bigger problem into smaller, more manageable pieces. Try solving the puzzle again, but this time [define](glossary://define) the `solveStair()` function and use it to collect a gem from a stairway. You'll need to move forward, collect a gem, turn around, and move back to the center."
