...
public func assessmentPoint() -> AssessmentResults {
    ...
    var hints = [
        "Remember, you need to move to each switch location and check to see how the switch is toggled. If it's closed, toggle it open.",
        "To check whether you should toggle the switch, use `isOnClosedSwitch` after `if` and before the `{ }` braces.",
        "To toggle a switch only when your charaacter is on a closed one, use `toggleSwitch()` between the `{ }` braces of the `if` statement.",
        ]
